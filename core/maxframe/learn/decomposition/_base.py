...
from maxframe.tensor import linalg
...
class _BasePCA(BaseEstimator, TransformerMixin, metaclass=ABCMeta):
    ...
    def get_covariance(self, session=None, execute=False, run_kwargs=None):
        ...
        if execute:
            cov.execute(session=session, **(run_kwargs or {}))
        return cov

    def get_precision(self, session=None, execute=False, run_kwargs=None):
        ...
        if self.n_components_ == 0:
            ...
            if execute:
                precision.execute(session=session, **(run_kwargs or {}))
            return precision
        if self.n_components_ == n_features:
            ...
            if execute:
                precision.execute(session=session, **(run_kwargs or {}))
            return precision
        # Scalar zero-noise fast path: avoid division by zero in the lemma.
        # Only check when noise_variance_ is a Python scalar; calling == on a
        # lazy Tensor would force execution via __bool__ and break execute=False.
        if (
            isinstance(self.noise_variance_, (int, float))
            and self.noise_variance_ == 0.0
        ):
            precision = linalg.inv(self.get_covariance())
            if execute:
                precision.execute(session=session, **(run_kwargs or {}))
    ...
    def transform(self, X, session=None, execute=False, run_kwargs=None):
        ...
        if execute:
            X_transformed.execute(session=session, **(run_kwargs or {}))
