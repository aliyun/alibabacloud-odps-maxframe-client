...
from maxframe import remote as mr
...
class PCA(_BasePCA):
    ...
    .. note:: ARPACK support (``svd_solver='arpack'``) is not implemented yet;
    ...
    svd_solver : string {'auto', 'full', 'arpack', 'randomized'}
        ...
        arpack :
            not implemented yet; raises ``NotImplementedError`` at fit time.
            Intended to run SVD truncated to n_components calling ARPACK
            solver via `scipy.sparse.linalg.svds`.
        randomized :
    ...
    *Martinsson, P. G., Rokhlin, V., and Tygert, M. (2011).
    ...
    def fit(self, X, y=None, session=None, execute=False, run_kwargs=None):
        ...
        """
        self._fit(X, session=session, execute=execute, run_kwargs=run_kwargs)
        return self
    ...
    def fit_transform(self, X, y=None, session=None, execute=False, run_kwargs=None):
        ...
        """
        U, S, _ = self._fit(X, session=session, execute=False, run_kwargs=run_kwargs)
        ...
        if execute:
            self._execute([U], session=session, run_kwargs=run_kwargs)
        return U

    def _execute(self, result, session=None, run_kwargs=None):
    ...
    def _fit(self, X, session=None, execute=True, run_kwargs=None):
        ...
        if self._fit_svd_solver == "auto":
            ...
            if max(X.shape) <= 500 or n_components == "mle":
                ...
                not isinstance(n_components, str)
                and n_components >= 1
                and n_components < 0.8 * min(X.shape)
            ):
        ...
        if self._fit_svd_solver == "full":
            ret = self._fit_full(
                X, n_components, session=session, run_kwargs=run_kwargs
            )
        ...
        if execute:
            self._execute(ret, session=session, run_kwargs=run_kwargs)
        return ret
    ...
    def _fit_full(self, X, n_components, session=None, run_kwargs=None):
        ...
        if n_components == "mle":
            ...
            raise ValueError(
                "n_components=%r is not a valid string value; "
                "only 'mle' is supported with svd_solver='full'" % n_components
            )
    ...
    def _score_samples(self, X, session=None, run_kwargs=None):
        ...
        precision = self.get_precision(
            session=session, run_kwargs=run_kwargs, execute=True
        ).fetch(session=session)
        log_like = -0.5 * (Xr * (mt.dot(Xr, precision))).sum(axis=1)
        log_like -= 0.5 * (n_features * log(2.0 * mt.pi) - fast_logdet(precision))
        return log_like

    def score_samples(self, X, session=None, execute=False, run_kwargs=None):
        ...
        """
        log_like = self._score_samples(X, session=session, run_kwargs=run_kwargs)
        if execute:
            log_like.execute(session=session, **(run_kwargs or {}))
        return log_like

    def score(self, X, y=None, session=None, execute=False, run_kwargs=None):
        ...
        """
        ret = mt.mean(self._score_samples(X, session=session, run_kwargs=run_kwargs))
        if execute:
