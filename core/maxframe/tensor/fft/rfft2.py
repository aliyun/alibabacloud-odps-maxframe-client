...
from maxframe.tensor.fft.core import TensorRealFFTN, TensorRealFFTNMixin, validate_fftn


class TensorRFFT2(TensorRealFFTN, TensorRealFFTNMixin):
    _op_type_ = opcodes.RFFT2
...
def rfft2(a, s=None, axes=(-2, -1), norm=None):
    ...
    cplx_type = np.complex_ if hasattr(np, "complex_") else np.complex128
    op = TensorRFFT2(shape=s, axes=axes, norm=norm, dtype=np.dtype(cplx_type))
    return op(a)
